...
## Версия 1.2.0 (2026-02-05)

### Новые функции

#### 1. История проверок (SQLite)
Создан модуль `src/database.py` для хранения истории:
- `save_name_check()` - сохранение проверок наименований
- `save_image_check()` - сохранение проверок изображений
- `get_name_checks()` / `get_image_checks()` - получение истории
- `get_statistics()` - статистика по проверкам
- `delete_check()` - удаление записей

#### 2. Экспорт отчётов
Добавлены функции экспорта в `src/app.py`:
- **Excel** - полный отчёт с форматированием (openpyxl)
- **PDF** - отчёт для печати (reportlab)

API endpoints:
- `GET /api/export/image/<id>/excel` - экспорт изображения в Excel
- `GET /api/export/image/<id>/pdf` - экспорт изображения в PDF
- `GET /api/export/name/<id>/excel` - экспорт наименования в Excel
- `GET /api/export/name/<id>/pdf` - экспорт наименования в PDF

#### 3. Страница истории
Новый шаблон `templates/history.html`:
- Статистика (всего, красные, жёлтые, зелёные)
- Вкладки для наименований и изображений
- Фильтрация по статусу риска
- Просмотр деталей в модальном окне
- Удаление записей
- Экспорт отчётов

#### 4. Публикация через Cloudflare Tunnel
Добавлена возможность публикации приложения в интернет:
- Установка: `brew install cloudflare/cloudflare/cloudflared`
- Запуск: `cloudflared tunnel --url http://localhost:5001`
- URL вида: `https://xxx-xxx.trycloudflare.com`

#### 5. Скрипты запуска
- `start.sh` - автоматический запуск сервера и туннеля
- `stop.sh` - остановка всех процессов

### Улучшения OCR
- Порог уверенности увеличен до 60%
- Фильтрация "мусорного" текста (короткие строки, без гласных)
- Фильтрация применяется в `app.py` при обработке результатов

### Обновления интерфейса
- Кнопка "История" в навигации
- Кнопки экспорта (Excel/PDF) в результатах проверки изображения
- Переменная `currentImageCheckId` для отслеживания текущей проверки
...
| GET | `/api/history/names` | История наименований |
| GET | `/api/history/images` | История изображений |
| GET | `/api/export/image/<id>/<format>` | Экспорт изображения |
| GET | `/api/export/name/<id>/<format>` | Экспорт наименования |
